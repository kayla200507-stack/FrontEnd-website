...
export const AcademicTab: React.FC<Props> = ({ form, onChange, onSave }) => (
  <div className="p-6 sm:p-8">
    <h3 className="text-xl font-bold text-slate-800 mb-6">Latar Belakang Akademik</h3>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-2">Jabatan Fungsional</label>
        <select value={form.jabatan} onChange={(e) => onChange("jabatan", e.target.value)} className={`${inputCls} bg-white`}>
          {["Asisten Ahli", "Lektor", "Lektor Kepala", "Guru Besar / Profesor"].map((v) => (
            <option key={v} value={v}>{v}</option>
          ))}
        </select>
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-2">Golongan / Pangkat</label>
        <input type="text" value={form.golongan} onChange={(e) => onChange("golongan", e.target.value)} className={inputCls} />
      </div>
      <div className="md:col-span-2">
        <label className="block text-sm font-medium text-slate-700 mb-2">Pendidikan Tertinggi</label>
